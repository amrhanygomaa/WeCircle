...
export function PremiumAnalyticsHome() {
  ...
  const feesData = ov?.feesCollectionTrend || [];
  ...
  const attendanceData = [
    { name: "Present", value: att.present || 0, color: "#3b82f6" },
    { name: "Absent", value: att.absent || 0, color: "#e2e8f0" },
    { name: "Late", value: att.late || 0, color: "#f59e0b" },
    { name: "Emergency", value: att.emergency || 0, color: "#10b981" }
  ];

  const pp = ov?.platformPerformance || { high: 0, medium: 0, low: 0 };
  ...
  const earningsData = ov?.earningsTrend || [];

  const topSubjectsData = ov?.topSubjectsTrend || [];

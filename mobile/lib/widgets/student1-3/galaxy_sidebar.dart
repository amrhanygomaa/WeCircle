...
class _GalaxySidebarState extends State<GalaxySidebar> with SingleTickerProviderStateMixin {
    ...
    return Drawer(
      ...
      child: Stack(
        children: [
          ...
          Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: isGroupB 
                  ? [const Color(0xFF020108), const Color(0xFF0A0E21), const Color(0xFF020108)]
                  : [const Color(0xFF03001C), const Color(0xFF16003B), const Color(0xFF03001C)],
              ),
            ),
          ),
          ...
          // 2. Optimized Background Design (Stars for 1-3, Grid for 4-6)
          Positioned.fill(
            child: RepaintBoundary(
              child: isGroupB 
                ...
                : const _OptimizedStarField(),
            ),
          ),

          // 4. Optimized Glass Overlay (No BackdropFilter for speed)
          Container(
            decoration: BoxDecoration(
              color: const Color(0xFF03001C).withValues(alpha: 0.6),
              border: const Border(
          ...
          SafeArea(
            child: Column(
              ...
              children: [
                ...
                _buildMenuItem(context, Icons.query_stats_rounded, 'إحصائياتي', () {}),
                _buildMenuItem(context, Icons.auto_awesome_rounded, 'إنجازات البطل', () {}),
                _buildMenuItem(context, Icons.settings_suggest_rounded, 'إعدادات السفينة', () {}),
  ...
  Widget _buildMenuItem(BuildContext context, IconData icon, String title, VoidCallback onTap, {bool isDestructive = false}) {
    ...
    final glowColor = isDestructive ? Colors.redAccent : const Color(0xFF00D2FF);
...
class _OptimizedStarField extends StatefulWidget {
  const _OptimizedStarField();
  @override
  State<_OptimizedStarField> createState() => _OptimizedStarFieldState();
}

class _OptimizedStarFieldState extends State<_OptimizedStarField> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return CustomPaint(
          painter: _StarPainter(progress: _ctrl.value),
        );
      },
    );
  }
...
class _StarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 60; i++) {
      ...
      final s = rng.nextDouble() * 2.0 + 0.5;
      final opacity = (rng.nextDouble() * 0.3 + 0.2) + (progress * 0.4);
      canvas.drawCircle(Offset(x, y), s, paint..color = Colors.white.withValues(alpha: opacity.clamp(0.0, 1.0)));
    }
  }
